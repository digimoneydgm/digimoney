...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("digimoney"))
        return false;
...
{
    // Convert digimoney:// to digimoney:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("digimoney://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "digimoney:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Digimoney.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Digimoney.lnk
...
{
    return GetAutostartDir() / "digimoney.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a digimoney.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Digimoney\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the digimoney app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add digimoney app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Digimoney-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  digimoney-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Digimoney-Qt"));
