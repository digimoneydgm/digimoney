...
int nAnonymizeDigimoneyAmount = 1000;
...
#else
    const char* pszModule = "digimoney";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Digimoney
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Digimoney
    // Mac: ~/Library/Application Support/Digimoney
    // Unix: ~/.digimoney
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Digimoney";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "digimoney.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "digimoneyd.pid"));
